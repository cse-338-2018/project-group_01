...
def run_plagiarism_analysis(query_text, data, plagiarism_threshold=0.8):
    ...
    for x in formated_result.iloc:
        json={'similarity_score': x["similarity"], 
                           'similarity_percentage': str(round(x["similarity"] *100)) + '%',
                            'is_plagiarism': is_plagiarism( x["similarity"], plagiarism_threshold),
                            'most_similar_article': x["abstract"], 
                            'article_submitted': query_text
                        }
        plagiarism_decision.append(json)
...
def predict(new_incoming_text, data_path):
    source_data = preprocess_data(data_path, 10000)
